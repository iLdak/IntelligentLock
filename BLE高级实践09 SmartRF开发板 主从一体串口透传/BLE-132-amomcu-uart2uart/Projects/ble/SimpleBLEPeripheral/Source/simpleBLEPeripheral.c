...
#define DEFAULT_DESIRED_MIN_CONN_INTERVAL     2//80   连接间隔与数据发送量有关， 连接间隔越短， 单位时间内就能发送越多的数据

// Maximum connection interval (units of 1.25ms, 800=1000ms) if automatic parameter update request is enabled
#define DEFAULT_DESIRED_MAX_CONN_INTERVAL     2//800   连接间隔与数据发送量有关， 连接间隔越短， 单位时间内就能发送越多的数据

// Slave latency to use if automatic parameter update request is enabled
#define DEFAULT_DESIRED_SLAVE_LATENCY         0

// Supervision timeout value (units of 10ms, 1000=10s) if automatic parameter update request is enabled
#define DEFAULT_DESIRED_CONN_TIMEOUT          10//1000  -各种原因断开连接后，超时并重新广播的时间:  100 = 1s
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
  ...
  // Setup the GAP Peripheral Role Profile
  {
    ...
    {       
        ...
        if(pSscanRspData)
        {
            ...
            GAPRole_SetParameter( GAPROLE_SCAN_RSP_DATA, pSscanRspDataLen, pSscanRspData );//设置GAP参数
        ...
        else
        {
            ...
            NPI_WriteTransport("ERR:pSscanRspData\r\n",19);
        }
    }
#endif
  ...
  {
    uint32 passkey = 1234567; // passkey "1234567"
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  if ( events & SBP_PERIODIC_EVT )
  {
    ...
    {
      osal_start_timerEx( simpleBLEPeripheral_TaskID, SBP_PERIODIC_EVT, SBP_PERIODIC_EVT_PERIOD );//定时产生事件
    }
...
static void ProcessPasscodeCB(uint8 *deviceAddr,uint16 connectionHandle,uint8 uiInputs,uint8 uiOutputs )
{
  ...
  passcode %= 10000000;  
#else
  passcode = str2Num(sys_config.pass, 7);
  passcode %= 10000000;
#endif
  ...
  {
    // 无需密码
    NPI_WriteTransport("no passcode", 11); 
